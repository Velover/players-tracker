...
export type PlayerTracker = ReturnType<typeof PlayersTracker.GetLocalTracker>;
...
export namespace PlayersTracker {
	class PlayerTracker {
		...
		constructor(player: Player) {
			...
			this.maid_.GiveTask(
				this.player_.CharacterAdded.Connect((model) =>
					this.OnCharacterAdded(model),
				),
			);
			this.maid_.GiveTask(
				this.player_.CharacterRemoving.Connect((model) =>
					this.OnCharacterRemoving(model),
				),
			);
		}
		...
		async AwaitAndLoadAnimation(
			animation: Animation,
			properties?: InstancePropertiesList<AnimationTrack>,
			can_be_dead: boolean = false,
		) {
			this.WaitTilCharacterWillAppear(can_be_dead);
	...
	class Character {
		...
		async Initialize() {
			...
			this.maid_.GiveTask(
				this.humanoid_.HealthChanged.Connect((health) =>
	...
	export function GetTrackerWithAssert(player: Player) {
		const player_tracker = GetTracker(player);
		assert(player_tracker, `Player tracker doesnt exist for ${player.Name}`);
